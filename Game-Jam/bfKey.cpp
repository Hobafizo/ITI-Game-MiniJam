...
bfKey::bfKey(b2Body* body, const sf::Vector2f size, unsigned int color) : _body(body)
{
	setSize(size);
...
bool bfKey::loadSpriteSheet(const std::string& filepath, int frameWidth, int frameHeight, int marginX, int marginY, int framePerLine, int numFrames, float curTime, float frameTime, bool autoResize)
{
	autoResize = false;
...
bool bfKey::updateSpriteSheet(int startIdx, int numFrames, float curTime, float frameTime)
{
...
void bfKey::setRotation(float angle)
{
    _body->SetTransform(getB2Position(), angle);
}

void bfKey::setScale(const sf::Vector2f scale)
{
...
void bfKey::setSfPosition(const sf::Vector2f pos)
{
	_shape.setPosition(pos);
}

void bfKey::setPositionFromTopLeft(const sf::Vector2f& topLeft)
{
...
void bfKey::setOrigin(const sf::Vector2f position)
{
	_shape.setOrigin(position);
}

void bfKey::setSize(const sf::Vector2f size)
{
	_shape.setSize(size);
}

void bfKey::setFillColor(unsigned int color)
{
	setFillColor(sf::Color(color));
}

void bfKey::setFillColor(const sf::Color color)
{
	_shape.setFillColor(color);
}

void bfKey::setOutlineColor(unsigned int color)
{
	setOutlineColor(sf::Color(color));
}

void bfKey::setOutlineColor(const sf::Color color)
{
	_shape.setOutlineColor(color);
}

void bfKey::updatePosition(float curTime)
{
    updateAnimation(curTime);
}

void bfKey::updateAnimation(float curTime)
{
    // --- ANIMATION SYNC ---
...
b2Body* bfKey::Body(void) const
{
	return _body;
}

sf::Drawable* bfKey::Drawable(void) const
{
	return (sf::Drawable*)&_shape;
}

b2Vec2 bfKey::getB2Position(void) const
{
	return _body->GetPosition();
}

sf::Vector2f bfKey::getSfPosition(void) const
{
